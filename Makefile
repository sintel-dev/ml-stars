...
.PHONY: lint-mlstars
lint-mlstars: ## check style with flake8 and isort
	flake8 mlstars tests
	isort -c --recursive mlstars tests

.PHONY: fix-lint
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find mlstars -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive mlstars
	isort --apply --atomic --recursive mlstars
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source mlstars -m pytest
	coverage report -m
	coverage html
